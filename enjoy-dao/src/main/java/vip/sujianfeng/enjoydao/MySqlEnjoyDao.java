...
import vip.sujianfeng.core.utils.GuidUtils;
 ...
 **/
public class MySqlEnjoyDao extends JdbcTbDao {

    public MySqlEnjoyDao(SqlAdapter sqlAdapter, DataSource dataSource, String dbName) {
        super(sqlAdapter, dataSource, dbName);
    }
    ...
    public String insertSql(TbTableSql table, List<Object> params) {
        ...
        if (table.getKeyType() == TbKeyType.AUTO_INC_INT){
            sql.appendFormater(" insert into %s(", mergeTableName(table.getTableName()));
            if (ConvertUtils.cInt(table.getId()) > 0){
                sql.appendFormater("`%s`,", table.getKeyField().getTableField());
            }
            ...
            sql.appendFormater("%s, %s, 0)", currTime, currTime);
        }else{
    ...
    public String updateSql(TbUpdateSql updateSql, List<Object> params) {
        ...
        long currTime = System.currentTimeMillis() / 1000;
        sql.appendFR(" update_time = %s", currTime);
    ...
    public String selectListSql(TbSelectSql tbSelectSql, String condition) {
        ...
        sql.appendFR(" where %s.state = 0 %s", tbSelectSql.getTableAlias(), condition);
        ...
        return this.changeSql(sql.toString());
    }
